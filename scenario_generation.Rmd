...
fit_arima_fourier <- auto.arima(load_ts, xreg = xreg_full)

# Forecast horizon and number of scenarios
horizon <- 12
nscen <- 10

# Generate simulated future predictors (Y from Cholesky step, already done)
# Let's reshape it to [horizon x nvar x nscen] to align with monthly steps
nvar <- 7
Y <- array(Y, dim = c(nvar, horizon, nscen))  # reshaped if needed

# Generate Fourier terms for forecast horizon
future_fourier <- fourier(load_ts, K = K, h = horizon)

# Prepare array for storing load scenarios
load_scenarios <- matrix(0, nrow = horizon, ncol = nscen)

# Generate load scenarios using future Fourier + predictor simulations
for (s in 1:nscen) {
...
plot(load_scenarios_exp[ , 1], type = "l", col = "gray", ylim = c(ymin, ymax),
     xlab = "Month", ylab = "Load (MW)", main = "ERCOT Load Scenarios (ARIMAX + Fourier)")
